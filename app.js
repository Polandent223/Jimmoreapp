...
function connectFirebase(){
  try{
    ...
    const cfg=parseFirebaseConfig($('firebaseConfig').value||state.config.firebase);
...
printOneInvoice=function(id){
  ...
  w.document.write(`<h1>${state.config.companyName}</h1><img src="assets/logo.jpg" width="220"><h2>Factura #${i.id}</h2><p>${i.date}</p><p><b>Cliente:</b> ${i.client}</p><p><b>RIF/CI:</b> ${i.clientDocument||'-'}</p><p><b>Dirección:</b> ${i.clientAddress||'-'}</p><table border=1 cellpadding=8>${i.items.map(x=>`<tr><td>${x.name}</td><td>${x.qty}</td><td>${money(x.price)}</td></tr>`).join('')}</table><h2>${money(i.total)} / ${bs(i.total*i.rate)}</h2>`);w.print();
}
...
login=function(){
  ...
  if(!u || String(u.password)!==pass){ alert('Usuario o contraseña incorrecta. Prueba usuario: '+({admin:'admin',vendedor:'vendedor',almacen:'almacen',taller:'taller',pintor:'pintor'}[role]||'admin')+' / clave: 1234'); return; }
...
(function(){
  ...
  document.addEventListener('DOMContentLoaded',()=>{ state.version='18.7 PRO SEGURIDAD Y RESPALDO'; localStorage.setItem('jimmoreDB',JSON.stringify(state)); setup183(); renderAll(); });
})();
...
(function fase185(){
  ...
  document.addEventListener('DOMContentLoaded',()=>{ state.version='18.7 PRO SEGURIDAD Y RESPALDO'; localStorage.setItem('jimmoreDB',JSON.stringify(state)); stamp185(); });
})();
...
(function fase186(){
  function setVersion186(){
    try{ state.version='18.7 PRO SEGURIDAD Y RESPALDO'; localStorage.setItem('jimmoreDB',JSON.stringify(state)); }catch(e){}
  }
  function firebaseLabel186(){
    try{
      ...
      if(small){ small.textContent='Versión 18.7 PRO SEGURIDAD Y RESPALDO'; }
    }catch(e){}
  }
  function stamp186(){
    try{
...
(function fase187(){
  ...
  window.importBackup187=function(ev){
    ...
    reader.onload=e=>{
      try{
        ...
        state.version='18.7 PRO SEGURIDAD Y RESPALDO';
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    try{
      state.version='18.7 PRO SEGURIDAD Y RESPALDO';
      localStorage.setItem('jimmoreDB',JSON.stringify(state));
      const small=document.querySelector('.status-strip-login div:nth-child(4) small'); if(small) small.textContent='Versión 18.7 PRO';
