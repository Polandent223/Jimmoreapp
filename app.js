...
function connectFirebase(){
  try{
    ...
    const cfg=normalizeFirebaseConfig(AUTO_FIREBASE_CONFIG || parseFirebaseConfig(($('firebaseConfig')&&$('firebaseConfig').value)||state.config.firebase));
...
saveConfig=function(){
  state.config={companyName:$('companyName').value,rif:$('companyRif').value,address:$('companyAddress').value,phone:$('companyPhone').value,firebase:autoFirebaseConfigText()};
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  if($('firebaseConfig')) $('firebaseConfig').value=autoFirebaseConfigText();
  state.config = state.config || {};
  state.config.firebase = autoFirebaseConfigText();
  localStorage.setItem('jimmoreDB', JSON.stringify(state));
  setTimeout(()=>{ try{ connectFirebase(); }catch(e){ console.error(e); } },800);
});
